...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cos = dot / (magnitude1 * magnitude2);
...
function roundToPowerOfTen(num, pow) {
  ...
  if (pow === 0) {
    ...
    result = Math.round(num / powerOfTen) * powerOfTen;
  }
...
function toNumber(value, def) {
  ...
  if (Number.isNaN(transformToNumber) === true) {
    result = def;
  } else if (
    typeof transformToNumber === 'number' ||
    transformToNumber instanceof Number
  ) {
